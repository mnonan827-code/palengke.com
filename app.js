...
const dbRefs = {
    ...
    chats: ref(database, 'chats') // <--- ADD THIS
};
...
function setupRealtimeListeners() {
    // ... existing listeners for products and orders ...
    
    // Chats Listener (Admin uses this to see all chats) <--- ADD THIS BLOCK
    onValue(dbRefs.chats, (snapshot) => {
        if (snapshot.exists()) {
            const chatsData = snapshot.val();
            // Convert object of chats into an array for easier rendering
            window.APP_STATE.chats = Object.keys(chatsData).map(key => ({
                id: key,
                ...chatsData[key]
            })).sort((a, b) => new Date(b.lastMessageAt) - new Date(a.lastMessageAt));

            // Re-render if admin is viewing the chat page
            if (window.APP_STATE.currentUser && window.APP_STATE.currentUser.role === 'admin' && window.APP_STATE.adminView === 'chats') {
                renderMain();
            }
            // Update unread count for customer view
...
// ===========================================
// CHAT / CUSTOMER SERVICE FUNCTIONS
// ===========================================

// Utility to find a chat thread for the current user
window.getUserChatThread = function() {
    if (!window.APP_STATE.currentUser) return null;
    const uid = window.APP_STATE.currentUser.uid;
    // Chat key is the user's UID
    return window.APP_STATE.chats.find(chat => chat.id === uid);
};

// Toggle the customer-facing chat modal
window.toggleChatModal = function(show) {
    ...
    if (modal) {
        ...
        // Mark as read when customer opens it
        if (!modal.classList.contains('hidden')) {
            markChatAsRead();
            renderCustomerChatModal();
        }
        updateChatUnreadBadge();
    }
};
...
window.markChatAsRead = async function() {
    const chat = window.getUserChatThread();
    if (chat && chat.customerUnreadCount > 0) {
        // Only update if there are unread messages for the customer
        await updateFirebase(`chats/${chat.id}`, { customerUnreadCount: 0 });
    }
};
...
window.updateChatUnreadBadge = function() {
    ...
    if (badge) {
        const chat = window.getUserChatThread();
        if (chat && chat.customerUnreadCount > 0) {
            badge.textContent = chat.customerUnreadCount;
...
// Send a message (used by both customer and admin)
    ...
    try {
        ...
        if (!currentChat) {
            ...
            updates = {
                ...
                adminUnreadCount: 1, // Admin is the recipient of the first message
        ...
        if (isCustomer && (window.APP_STATE.currentUser.role !== 'admin' || window.APP_STATE.adminView !== 'chats')) {
             updateAdminChatTabBadge(window.APP_STATE.chats.filter(c => c.adminUnreadCount > 0).length + 1);
        }
    } catch (error) {
...
window.renderCustomerChatModal = function() {
    const chatModalBody = document.getElementById('customer-chat-messages');
    if (!chatModalBody) return;

    const chat = window.getUserChatThread();
    const currentUserId = window.APP_STATE.currentUser?.uid;

    if (!chat) {
        chatModalBody.innerHTML = `<div class="p-4 text-center text-gray-500">Start a new conversation!</div>`;
        return;
    }
    ...
    const messagesHtml = chat.messages.map(m => {
        ...
        const bgClass = isSelf ? 'bg-lime-600 text-white' : 'bg-gray-200 text-gray-800';
        const cornerClass = isSelf ? 'rounded-br-none' : 'rounded-bl-none';
        const senderName = isSelf ? 'You' : m.sender;
        const time = new Date(m.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });

        return `
            <div class="flex ${alignClass} mb-3">
                <div class="max-w-xs md:max-w-md">
                    <div class="text-xs text-gray-500 mb-1 ${isSelf ? 'text-right' : 'text-left'}">${senderName} • ${time}</div>
                    <div class="p-3 ${bgClass} rounded-xl ${cornerClass} shadow-md">
                        ${m.message}
                    </div>
                </div>
            </div>
    ...
    chatModalBody.innerHTML = messagesHtml;
    // Scroll to bottom
    setTimeout(() => {
...
// Admin view switcher for Chats
window.switchToAdminChatView = function() {
    window.APP_STATE.adminView = 'chats';
    window.APP_STATE.selectedChatId = null; // Clear selected chat when going to list view
    renderMain();
};

window.updateAdminChatTabBadge = function(count) {
    const badge = document.getElementById('admin-chats-badge');
    if (badge) {
        if (count > 0) {
...
// ===========================================
// ADMIN CHATS VIEW RENDERING
// ===========================================
window.renderAdminChatsView = function() {
    const allChats = window.APP_STATE.chats;
    const selectedChat = window.APP_STATE.selectedChatId ? allChats.find(c => c.id === window.APP_STATE.selectedChatId) : null;

    const chatListHtml = allChats.map(chat => {
        const unreadCount = chat.adminUnreadCount || 0;
        const isSelected = chat.id === window.APP_STATE.selectedChatId;
        const lastMsgTime = new Date(chat.lastMessageAt).toLocaleString('en-US', { hour: '2-digit', minute: '2-digit', day: '2-digit', month: 'short' });
        
        return `
            <div onclick="window.APP_STATE.selectedChatId = '${chat.id}'; renderMain(); adminMarkChatAsRead('${chat.id}')"
                 class="p-3 border-b cursor-pointer transition duration-150 ${isSelected ? 'bg-lime-100 border-l-4 border-lime-600' : 'hover:bg-gray-50'}">
                <div class="flex justify-between items-center">
                    <h4 class="font-semibold text-gray-800">${chat.customerName}</h4>
                    <span class="text-xs text-gray-500">${lastMsgTime}</span>
                </div>
                <div class="flex justify-between items-center mt-1">
                    <p class="text-sm text-gray-600 truncate mr-2">${chat.lastMessage}</p>
                    ${unreadCount > 0 ? `<span class="bg-red-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">${unreadCount}</span>` : ''}
                </div>
            </div>
        `;
    }).join('');

    const chatMessagesHtml = selectedChat ? selectedChat.messages.map(m => {
        const isSelf = m.senderId === 'admin-id'; // Admin is self
        const alignClass = isSelf ? 'justify-end' : 'justify-start';
        const bgClass = isSelf ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-800';
        const cornerClass = isSelf ? 'rounded-br-none' : 'rounded-bl-none';
        const senderName = isSelf ? 'Admin' : m.sender;
        const time = new Date(m.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', day: '2-digit', month: 'short' });
        ...
        `;
    }).join('') : '<div class="p-4 text-center text-gray-500">Select a chat from the left to view the conversation.</div>';

    const chatWindow = selectedChat ? `
        <div class="flex flex-col h-full bg-white rounded-xl shadow-md border">
            <div class="p-4 border-b">
                <h3 class="font-bold text-lg">${selectedChat.customerName}</h3>
                <p class="text-sm text-gray-500">${selectedChat.customerEmail}</p>
            </div>
            <div id="admin-chat-messages" class="flex-1 p-4 overflow-y-auto custom-scroll space-y-3">
                ${chatMessagesHtml}
            </div>
            <div class="p-4 border-t bg-gray-50">
                <div class="flex gap-2">
                    <input type="text" id="admin-chat-reply-input" placeholder="Type your reply..." 
                           class="flex-1 p-2 border rounded-lg focus:ring-lime-600 focus:border-lime-600 text-sm"
                           onkeydown="if(event.key==='Enter') sendChatMessage(false)">
                    <button onclick="sendChatMessage(false)" class="bg-lime-600 text-white p-2 rounded-lg hover:bg-lime-700 transition">
                        <i data-lucide="send" class="w-5 h-5"></i>
                    </button>
                </div>
            </div>
        </div>
    ` : `<div class="flex items-center justify-center h-full text-xl text-gray-400">Select a Chat to Start</div>`;

    // Function to scroll chat window to bottom after rendering
    setTimeout(() => {
    ...
    return `
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 h-[80vh]">
            <div class="md:col-span-1 bg-white rounded-xl shadow-md border overflow-y-auto custom-scroll">
...
window.adminMarkChatAsRead = async function(chatId) {
    const chat = window.APP_STATE.chats.find(c => c.id === chatId);
    if (chat && chat.adminUnreadCount > 0) {
        await updateFirebase(`chats/${chatId}`, { adminUnreadCount: 0 });
    }
    // Update the tab badge immediately based on all chats
    const newUnreadCount = window.APP_STATE.chats.filter(c => c.id !== chatId && c.adminUnreadCount > 0).length;
    updateAdminChatTabBadge(newUnreadCount);
};
// Add a function to update the admin chat tab badge based on total unread chats
window.updateAdminChatTabBadge = function(count) {
    const badge = document.getElementById('admin-chats-badge');
    if (badge) {
        if (count > 0) {
...
window.renderAdminArea = function() {
    ...
    const navHtml = `
        <div class="admin-nav bg-white rounded-xl shadow-sm p-4 flex gap-4 mb-6 sticky top-4 md:top-0 z-30 overflow-x-auto custom-scroll">
            ...
            <button onclick="window.APP_STATE.adminView = 'products'; renderMain()" 
                class="btn-ghost ${window.APP_STATE.adminView === 'products' ? 'bg-lime-600 text-white hover:bg-lime-700' : 'hover:bg-gray-50'}">
                <i data-lucide="store" class="w-5 h-5"></i> Products
            </button>
            <button onclick="window.APP_STATE.adminView = 'orders'; renderMain()" 
                class="btn-ghost ${window.APP_STATE.adminView === 'orders' ? 'bg-lime-600 text-white hover:bg-lime-700' : 'hover:bg-gray-50'}">
                <i data-lucide="package" class="w-5 h-5"></i> Regular Orders
            </button>
            <button onclick="window.APP_STATE.adminView = 'preorders'; renderMain()" 
                class="btn-ghost ${window.APP_STATE.adminView === 'preorders' ? 'bg-lime-600 text-white hover:bg-lime-700' : 'hover:bg-gray-50'}">
                <i data-lucide="calendar" class="w-5 h-5"></i> Pre-Orders
            </button>
            
            <button onclick="switchToAdminChatView()" 
                class="btn-ghost ${window.APP_STATE.adminView === 'chats' ? 'bg-lime-600 text-white hover:bg-lime-700' : 'hover:bg-gray-50'} relative">
                <i data-lucide="message-square" class="w-5 h-5"></i> Chats
                <span id="admin-chats-badge" class="absolute top-[-5px] right-[-5px] text-xs px-1.5 py-0.5 bg-red-600 text-white rounded-full ${unreadChatsCount > 0 ? '' : 'hidden'}">${unreadChatsCount}</span>
            </button>
            <button onclick="window.APP_STATE.adminView = 'users'; renderMain()" 
                class="btn-ghost ${window.APP_STATE.adminView === 'users' ? 'bg-lime-600 text-white hover:bg-lime-700' : 'hover:bg-gray-50'}">
                <i data-lucide="users" class="w-5 h-5"></i> Users
            </button>
            <button onclick="window.APP_STATE.adminView = 'settings'; renderMain()" 
                class="btn-ghost ${window.APP_STATE.adminView === 'settings' ? 'bg-lime-600 text-white hover:bg-lime-700' : 'hover:bg-gray-50'}">
...
window.renderMain = async function() {
    ...
    icons(); 
    // This needs to be called after rendering to ensure the badge is up to date
    if (window.APP_STATE.currentUser && window.APP_STATE.currentUser.role === 'admin') {
         const unreadChatsCount = window.APP_STATE.chats.filter(c => c.adminUnreadCount > 0).length;
         window.updateAdminChatTabBadge(unreadChatsCount);
    }
};
